...
- **Tactics trainer** — a Puzzles mode with ~24,000 curated Lichess puzzles, a local tactics rating, streaks, a **lifetime first-try win rate**, a **per-puzzle countdown that scores each solve out of 10** (the more clock you leave, the more points — a correct solve is always worth at least 4; a miss or timeout is 0), with points tallied **per session**, plus theme/difficulty filters and hints (see below). **Play from here** turns any puzzle into a starting position: hand it straight to the Play tab and continue it as a normal, fully-coached game against the bot (you take the side to move) — instead of only solving the scripted line.
...
when you solve it — points accumulate for the session. It tracks an Elo-style
tactics rating, a streak, and a **lifetime first-try win rate** in
`localStorage` (all preserved across sessions), filters by theme and difficulty,
and offers hints. Retrying a puzzle is untimed practice and never re-scores.
