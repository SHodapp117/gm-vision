...
- **Pre-game setup** — play as White or Black, a bot ELO slider (600–3200), and a 15-opening selector (Ruy Lopez, Italian, Scotch, Vienna, Sicilian Najdorf, French, Caro-Kann, Scandinavian, Pirc, Queen's Gambit Declined, Slav, King's Indian, Nimzo-Indian, London, English). Every main line is validated move-by-move against chess.js. **Set the bot's opening independently** — leave it on *Mirror my line* to rehearse your opening against its own book, or pick a *different* line so the bot answers you with an opponent's repertoire. Because the two lines only overlap so far, the setup panel shows the **shared line** ("1.e4 e5 2.Nf3 Nc6 — you'll be coached this far…") and flags pairings that diverge immediately, so you know before you start how long the guide will follow you.
...
- **Nemesis bot — built from your own games.** Once you've analyzed more than 20 of your imported games, **Build my Nemesis** reads them and assembles a bot tuned to *your* weaknesses. It's honest about what a bot can actually control: it makes you play your **weaker colour** (by win rate) and **ramps its strength in the phase you crumble in** — a fair ~your-rating fight in the opening/middlegame, then a jump of ~250 Elo once the endgame is reached (or wherever your blunders cluster), so the hard reps land exactly on your weak spot. A **scouting report** shows what it's doing: chips marked **TARGETS** (actively exploited — colour, phase) versus **KNOWS** (your most-missed tactic and worst opening, surfaced as intel rather than faked into forced play, because you choose your own opening moves — the bot can't). Change colour or the ELO slider to dismiss it. Needs the Games tab (import + analyze).
- **Tactics trainer** — a Puzzles mode with ~24,000 curated Lichess puzzles scored by a **per-puzzle countdown**: each solve is worth up to 10 points by how much clock you leave (a correct solve is always worth at least 4; a miss or timeout is 0), and **points are your score, tallied per session**. It also keeps a **streak** and a **lifetime first-try win rate**, with theme/difficulty filters and hints (see below). **Play from here** turns any puzzle into a starting position: hand it straight to the Play tab and continue it as a normal, fully-coached game against the bot (you take the side to move) — instead of only solving the scripted line.
- **Chess.com import** — a **Games** tab that pulls your real games from the [Chess.com public API](https://www.chess.com/news/view/published-data-api): enter your username, pick a month range, and import. Games are deduped by their Chess.com id and stored locally (IndexedDB), so re-importing only adds what's new. Browse and filter them (result / time control / opponent / rating), replay any game on the board, **Analyze** it with the bundled Stockfish to badge every move (good/best/brilliant → inaccuracy/mistake/blunder) and show a **Key moments** breakdown (your move vs. the engine's best, drawn on the board), and **Play from here** to continue any position against the bot. A **Coach Report** meta-analyzes your whole history — an instant profile plus prioritized, actionable improvement tips (weak colour/time-control/opening, how you tend to lose, recurring mistakes), each backed by numbers and clickable example games; an "Analyze N games" batch deepens it. Your analysed blunders become **personalized puzzles** — "Practice N puzzles from your mistakes" drops them into the Puzzles tab's **My mistakes** mode (find the move you missed, in the position from your own game), where a **spaced-repetition review loop** schedules each one (solve it and it comes back later; miss it and it returns now), tracks per-motif accuracy, and serves your weakest tactic first — with a "Review plan" card showing what's due, what you've mastered, and your streak. Fully client-side and read-only — no password, no account linking. Details in [`src/chesscom/README.md`](src/chesscom/README.md).
...
A second mode (the **Puzzles** tab) drills tactics from ~24,000 curated
[Lichess](https://database.lichess.org/) puzzles (CC0), balanced across rating
bands and themes. Each puzzle runs against a **countdown** (its length scales
with the puzzle's rating) and is scored **out of 10** by how much clock is left
when you solve it — those **points are the score**, tallied for the session. It
also tracks a **streak** and a **lifetime first-try win rate** in `localStorage`
(preserved across sessions), filters by theme and difficulty, and offers hints.
A hidden tactics rating still adapts the "near your level (auto)" difficulty, but
it isn't shown — the timed points replaced it. Retrying a puzzle is untimed
practice and never re-scores.
