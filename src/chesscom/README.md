...
| `nemesis.ts` | `buildNemesis(games)` — a Play-tab bot built from your analysed games (needs >20). Picks your weaker **colour** and the **phase** you blunder in, so the runtime bot (`nemesisElo(plan, phaseOfFen(fen))`) plays a fair fight where you're solid and ~250 Elo stronger where you crack. Your worst opening + most-missed motif ride along as intel only — a bot can't force which opening *you* play. Pure/deterministic. |
